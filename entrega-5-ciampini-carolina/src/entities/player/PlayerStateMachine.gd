...
func _setup() -> void:
	...
	if character != null:
		if character.has_signal(&"hit") and not character.hit.is_connected(notify_hit):
			character.hit.connect(notify_hit)
		if character.has_signal(&"healed") and not character.healed.is_connected(notify_healed):
			character.healed.connect(notify_healed)
